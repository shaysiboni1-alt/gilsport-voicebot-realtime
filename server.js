...
wss.on("connection", async (twilioWs, req) => {
  ...
  // Unified prompt sender for both providers.
  if (PROVIDER_MODE === "gemini") {
    ...
    try {
      ...
      botTurnActive = true;
      logDebug(connId, `gemini turnComplete prompt purpose=${purpose || "no-tag"} text=${text}`);
    } catch (_) {}
    return;
  }
...
function detectGenderPreference(text) {
  ...
  async function endCall(reason, closingMessage) {
    ...
    const intentKey = String(parsedLead?.intent || "").toLowerCase();
    // Message leads: allow FINAL without full_name, because caller-id/phone + content is sufficient for callback.
    const isFullLead = intentKey === "message"
      ? !!(hasContent && phoneExists)
      : !!(hasName && hasContent && phoneExists);
