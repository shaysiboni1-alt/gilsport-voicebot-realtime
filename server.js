...
function sanitizeLeadNotes(notes) {
  const raw = safeStr(notes);
  if (!raw) return null;

  // Remove system-like negative statements that should not be sent to CRM unless explicitly provided by the caller.
  // Example patterns observed: "נמסר מספר מוביל: לא", "נמסר מספר יבואן: לא (חסום)".
  const parts = raw
    .split(/\n|\/|;|\|/g)
    .map((p) => safeStr(p))
    .filter(Boolean);
  ...
  for (const p of parts) {
    const norm = normalizeTextLoose(p);
    if (!norm) continue;

    // Drop "not provided"/"blocked" markers for importer/delivery numbers.
    if (norm.includes(normalizeTextLoose("נמסר מספר מוביל")) || norm.includes(normalizeTextLoose("נמסר מספר יבואן"))) {
      if (norm.includes(normalizeTextLoose("לא")) || norm.includes(normalizeTextLoose("חסום")) || norm.includes(normalizeTextLoose("blocked"))) {
        continue;
      }
    }

    // Also drop generic "לא נמסר" sentences in notes (hard rule: avoid missing-info notes).
    if (norm.includes(normalizeTextLoose("לא נמסר"))) continue;
...
wss.on("connection", async (twilioWs, req) => {
  ...
  async function endCall(reason, closingMessage) {
    ...
    let call_status = mapCallStatus(reason, plannedEnd);
    if (isFullLead && call_status === "abandoned") call_status = "completed";
