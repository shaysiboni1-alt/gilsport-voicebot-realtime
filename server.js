...
function formatDigitsForTts(digits) {
  const d = extractDigits(digits);
  if (!d) return "";
  return d.split("").join(" ");
}

function coerceLeadFields(parsed) {
  ...
  const out = {
    is_lead: !!pickFirst(["is_lead", "isLead", "lead", "ליד", "is_lead?", "isLead?"]),
    intent: pickFirst(["intent", "route", "מסלול", "כוונה", "סיבת_פנייה", "סיבתפנייה"]) || "unknown",
    full_name: pickFirst(["full_name", "fullname", "שם_מלא", "שםמלא", "שם", "לקוח_שם"]) || null,
    phone_number: pickFirst(["phone_number", "phone", "טלפון", "טלפון_לחזרה", "טלפוחזרה", "טלפון_לשיחה", "טלפון_לשיחה", "מספר"]) || null,
    prefers_caller_id: pickFirst(["prefers_caller_id", "prefer_caller_id", "caller_id", "מספר_מזוהה", "prefersCallerId"]) ?? null,
    brand: pickFirst(["brand", "מותג"]) || null,
    model: pickFirst(["model", "דגם"]) || null,
    reason: pickFirst(["reason", "סיבה", "סיבת_פנייה", "סיבתפנייה"]) || null,
    notes: pickFirst(["notes", "note", "הערות", "סיכום", "summary"]) || null,
  };

  const intentRaw = String(out.intent || "unknown").toLowerCase();
  if (["sales", "support", "delivery", "message", "unknown"].includes(intentRaw)) {
    out.intent = intentRaw;
  } else {
    // Common Hebrew fallbacks
    if (intentRaw.includes("מכיר") || intentRaw.includes("רכיש")) out.intent = "sales";
    else if (intentRaw.includes("תקלה") || intentRaw.includes("שירות") || intentRaw.includes("אחריות")) out.intent = "support";
    else if (intentRaw.includes("משלוח") || intentRaw.includes("אספק")) out.intent = "delivery";
    else if (intentRaw.includes("הודעה") || intentRaw.includes("מסר")) out.intent = "message";
    else out.intent = "unknown";
  }

  if (out.phone_number) out.phone_number = extractDigits(out.phone_number);
  return out;
}
...
`.trim();
  ...
  try {
    ...
    // Normalize phone (digits-only) when enabled
    if (MB_FORCE_DIGITS_PHONE && coerced.phone_number) {
      const normalized = normalizePhoneNumber(coerced.phone_number, null);
      coerced.phone_number = normalized || extractDigits(coerced.phone_number) || null;
    }

    logInfo(tag, "Lead parsed.", coerced);
    return coerced;
  } catch (err) {
      ...
      case "response.audio_transcript.done": {
        ...
        if (text) {
          ...
          checkBotClosing(text);
	          ...
	          // Safety-net: if we already captured a phone number from the caller, and the bot
	          // tried to repeat a number but the digits do not match, issue a short correction.
	          const cap = extractDigits(capturedPhoneIL);
	          if (!phoneCorrectionSent && cap && (text.includes("טלפון") || text.includes("מספר"))) {
	            const said = extractDigits(text);
	            // Only intervene if the bot actually spoke a digit string and it doesn't match.
	            if (said.length >= 7 && said !== cap) {
	              phoneCorrectionSent = true;
	              const capSpaced = formatDigitsForTts(cap);
	              sendModelPrompt({
	                purpose: "phone_correction",
	                text: `סליחה, תיקון קטן—המספר לחזרה הוא: ${capSpaced}. נכון? אנא ענו "כן" או תנו מספר אחר.`
	              }).catch(() => {});
	            }
	          }
        }
